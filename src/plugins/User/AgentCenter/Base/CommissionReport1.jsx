...
class CommissionReport extends Component {
  ...
  componentWillUnmount() {
    this.dispatch({
      ...
      payload: [
        'myCommissionHistory', 'status', 'totalBetsSum', 'totalCommission', 'taskIdentifier'
      ]
    });
    this.dispatch({
  ...
  onBackClick() {
    this.dispatch({ type: 'userModel/initializeState', payload: ['taskIdentifier'] });
  }
  ...
  renderCommissionDetail() {
    const {
      taskIdentifier, awaitingResponse, commissionDetail, commissionDetailCount
    } = this.props;
    const listProps = {
      taskIdentifier,
      awaitingResponse,
  ...
  renderCommissionList() {
    ...
    const {
      status, dayCounts, myCommissionHistory, totalBetsSum, totalCommission,
      startTime, endTime,
    } = this.props;
    const listProps = {
...
const mapStateToProps = ({ userModel, dataTableModel }) => {
