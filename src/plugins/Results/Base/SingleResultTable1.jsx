...
import ruleIcon from 'assets/image/allIcon/ruleIcon.svg';
import trendPrizeIcon from 'assets/image/allIcon/trendPrizeIcon.svg';
import {EllipsisLoader, LotteryBalls, MDIcon} from 'components/General';
import css from 'styles/results/Base/singleResult1.less';
    ...
    return (
      <div className={css.gameInfo}>
        <div className={css.gameInfoRow}>
          ...
          {withTrend ? (
            <button type="button" className={css.predictBtn}>
              <Link
                target="trendPage"
                disabled={!withTrend}
                to={`/trends/${gameUniqueId}`}>
                <SVG className={css.trendPrizeIcon} src={trendPrizeIcon} />
                <span>号码走势</span>
              </Link>
            </button>
          ) : (
            <button type="button" className={css.predictDisabledBtn}>
              <SVG
                className={css.trendPrizeIconDisabled}
                src={trendPrizeIcon}
              />
              <span className={css.wordPredict}>暂不支持</span>
            </button>
          )}
          <button type="button" className={css.rulesBtn}>
            <Link
              target="_blank"
              to={`/instructions?gameUniqueId=${gameUniqueId}`}>
              <SVG className={css.ruleIcon} src={ruleIcon} />
              <span className={css.wordPredict}>玩法规则</span>
            </Link>
          </button>
        </div>
